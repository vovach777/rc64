/* =========================================================================
 * RC32SUB DECODE — Subbotin carryless range coder (byte-oriented)
 *
 * Usage:
 *   rc32sub_decode <input_file.rc32sub> <output_file> [no_progress]
 * ========================================================================= */
...
int main(int argc, char **argv) {
    ...
    if (argc < 3 || argc > 4) {
        fprintf(stderr, "Usage: %s <input.rc32sub> <output> [no_progress]\n", argv[0]);
        return 1;
    }
    ...
    if (sig[0] != 'r' || sig[1] != 's') {
        fprintf(stderr, "bad signature (expected 'rs', got %c%c)\n", sig[0], sig[1]);
        zpl_file_close(&fin); return 1;
    }
    ...
    if (is_rle) {
        ...
        zpl_file_close(&fout);
        printf("DECODE-RC32SUB OK (RLE, sym=0x%02X, len=%zu)\n", rle_sym, n);
        return 0;
    }
    ...
    rc32sub_dec_t rd;
    rc32sub_dec_init(&rd, stream, (size_t)ac_data_len);
    ...
    while (i != n) {
        ...
        for (uint32_t k = 0; k < block_size; k++) {
            uint16_t cum = (uint16_t)rc32sub_dec_get_cum(&rd);
            if (cum >= RC32SUB_TOTAL) cum = RC32SUB_TOTAL - 1;
            uint16_t cum_lo, freq;
            uint8_t sym = model12_find(&M, cum, &cum_lo, &freq);
            rc32sub_dec_step(&rd, cum_lo, freq);
            out_buf[k] = sym;
        }
    ...
    printf("DECODE-RC32SUB OK\n");
    printf("  engine    : Subbotin carryless RC, 32-bit state, 8-bit bytes\n");
