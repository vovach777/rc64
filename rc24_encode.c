/* =========================================================================
 * RC24 ENCODE — Subbotin carryless range coder (byte-oriented)
 *
 * Usage:
 *   rc24_encode <input_file> <output_file.rc24> [no_progress]
 *
 * .rc24 format:
...
int main(int argc, char **argv) {
    if (argc < 3 || argc > 4) {
        fprintf(stderr, "Usage: %s <input_file> <output_file.rc24> [no_progress]\n", argv[0]);
        return 1;
    }
    ...
    if (is_rle) {
        zpl_file_close(&fout);
        printf("ENCODE-RC24 OK (RLE, sym=0x%02X)\n", rle_sym);
    ...
    rc24_enc_t rc;
    rc24_enc_init(&rc, buf, buf_bytes);
    ...
    size_t nbytes = rc24_enc_size(&rc);
    if (nbytes > buf_bytes) {
    ...
    printf("ENCODE-RC24 OK\n");
    printf("  engine    : Subbotin carryless RC, 24-bit state, 8-bit bytes, TOTAL=4096\n");
    printf("  input     : %s\n", argv[1]);
