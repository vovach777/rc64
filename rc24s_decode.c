/* =========================================================================
 * RC24S DECODE — 24-bit carryless RC, N=4 interleaved streams, SHARED BUFFER.
 *
 * Usage: rc24s_decode <input.rc24s> <output> [no_progress]
 * ========================================================================= */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
int main(int argc, char **argv) {
    ...
    if (argc<3||argc>4) { fprintf(stderr,"Usage: %s <in.rc24s> <out> [no_progress]\n",argv[0]); return 1; }
    int no_progress = (argc==4 && !strcmp(argv[3],"no_progress"));

    zpl_file fin; if (zpl_file_open(&fin,argv[1])) { fprintf(stderr,"open\n"); return 1; }
    uint8_t sig[4]; if(!zpl_file_read(&fin,sig,4)){fprintf(stderr,"sig\n");zpl_file_close(&fin);return 1;}
    if (sig[0]!='r'||sig[1]!='4'||sig[2]!='s') { fprintf(stderr,"bad sig\n"); zpl_file_close(&fin); return 1; }
    int is_rle = sig[3] & 1;
    uint64_t orig=0; if(!zpl_file_read(&fin,&orig,8)){fprintf(stderr,"len\n");zpl_file_close(&fin);return 1;}
    ...
    if (is_rle) {
        ...
        uint8_t rs=sig[3]; memset(out_buf,rs,OUT_BUF_SIZE);
        size_t rem=n; while(rem>0){size_t b=rem<OUT_BUF_SIZE?rem:OUT_BUF_SIZE; if(!zpl_file_write(&fo,out_buf,b)){fprintf(stderr,"wr\n");return 1;} rem-=b;}
        zpl_file_close(&fo); printf("DECODE-RC24S OK (RLE)\n"); return 0;
    }
    ...
    /* Precompute per-cum symbol/cum/freq tables to remove model12_find from hot loop. */
    ...
    zpl_file fout; if(zpl_file_create(&fout,argv[2])){fprintf(stderr,"out\n");free(shared);return 1;}

    uint32_t dlow[N], drange[N], dcode[N]; size_t dpos[N];
    for (int s=0; s<N; s++) {
    ...
    while (i != n) {
        ...
        for (uint32_t k=0; k<blk; k++) {
            int s = (int)((i+k) % N);
            drange[s] /= RC24_TOTAL;
            uint32_t cum = rc_fast_div24(dcode[s] - dlow[s], drange[s]);
            if (cum>=RC24_TOTAL) cum=RC24_TOTAL-1;
            out_buf[k] = dec_sym_tbl[cum];
            dlow[s] += dec_cum_tbl[cum]*drange[s]; drange[s] *= dec_freq_tbl[cum];
            while ((dlow[s]^(dlow[s]+drange[s]))<RC24_TOP ||
                   (drange[s]<RC24_BOT&&((drange[s]=-dlow[s]&(RC24_BOT-1)),1))) {
                if (drange[s]==0) drange[s]=RC24_BOT-1;
                dcode[s]=((dcode[s]<<8) | (uint32_t)(dpos[s]<max_len ? shared[dpos[s]*N+s] : 0)) & RC24_MASK;
                drange[s]<<=8; dlow[s]=(dlow[s]<<8)&RC24_MASK;
                dpos[s]++;
            }
        }
    ...
    printf("DECODE-RC24S OK\n");
    printf("  engine    : 24-bit carryless RC, N=%d super-interleaved\n", N);
